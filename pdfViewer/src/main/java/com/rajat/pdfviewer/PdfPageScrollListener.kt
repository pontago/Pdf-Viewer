...
internal class PdfPageScrollListener(
    ...
    private val updatePosition: (Int, Int, Int) -> Unit
    ...
    override fun onScrolled(recyclerView: RecyclerView, dx: Int, dy: Int) {
        ...
        var positionToUse = firstVisible
        val isPositionChanged = firstVisible != lastVisible || firstComplete != lastComplete
        if (isPositionChanged) {
            positionToUse = if (firstComplete != RecyclerView.NO_POSITION) {
                firstComplete
            } else {
                firstVisible
            }
        }
        val firstItemView: View? = layoutManager.findViewByPosition(positionToUse)
        updatePosition(positionToUse, firstItemView?.left ?: 0, firstItemView?.top ?: 0)
    }
